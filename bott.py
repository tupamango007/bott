...
LOG_CHANNEL_ID = int(os.environ.get("1477285499059310612", 0))   # канал для логов 
...
def format_text_with_entities(text, entities):
    ...
    for entity in sorted_entities:
        ...
        # url и другие типы оставляем как есть
    return ''.join(result)
...
# ---------- Слеш-команды Discord ----------
@discord_client.tree.command(name="pause", description="Приостановить пересылку")
async def slash_pause(interaction: discord.Interaction):
    global bot_paused
    if interaction.user.guild_permissions.administrator:
        bot_paused = True
        await interaction.response.send_message("⏸ Пересылка сообщений приостановлена.")
...
async def slash_resume(interaction: discord.Interaction):
    ...
    if interaction.user.guild_permissions.administrator:
        bot_paused = False
        await interaction.response.send_message("▶ Пересылка сообщений возобновлена.")
...
async def slash_ban(interaction: discord.Interaction, user_id: str):
    ...
    if uid not in IGNORE_USERS:
        ...
        await interaction.response.send_message(f"✅ Пользователь {uid} добавлен в чёрный список.")
        await send_log(f"Забанен пользователь Telegram {uid} (slash)", "INFO")
    else:
        await interaction.response.send_message(f"⚠️ Пользователь {uid} уже в чёрном списке.")
...
async def slash_unban(interaction: discord.Interaction, user_id: str):
    ...
    if uid in IGNORE_USERS:
        ...
        await interaction.response.send_message(f"✅ Пользователь {uid} удалён из чёрного списка.")
        await send_log(f"Разбанен пользователь Telegram {uid} (slash)", "INFO")
    else:
        await interaction.response.send_message(f"⚠️ Пользователь {uid} не найден в чёрном списке.")
